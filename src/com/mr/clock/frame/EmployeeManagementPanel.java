...
public class EmployeeManagementPanel extends JPanel {//员工管理面板
    ...
    private void init() {//组件初始化
        ...
        Iterator<Employee> it = Session.EMP_SET.iterator();// 创建员工结合的迭代器
    ...
    private void addListener() {//为组件添加监听
        add.addActionListener(new ActionListener() {// 录入新员工按钮的事件
            public void actionPerformed(ActionEvent e) {
                parent.setPanel(new AddEmployeePanel(parent));// 主窗体切换到添加新员工面板
            }
        });
        delete.addActionListener(new ActionListener() { // 删除员工按钮的事件
            public void actionPerformed(ActionEvent e) {
                int selecRow = table.getSelectedRow();// 获取表格选中的行索引
                if (selecRow != -1) {// 如果有行被选中
                    int deleteCode = JOptionPane.showConfirmDialog(parent, "确定删除该员工？", "提示！",JOptionPane.YES_NO_OPTION);// 弹出选择对话框，并记录用户选择
                    if (deleteCode == JOptionPane.YES_OPTION) {// 如果用户选择确定
                        String id = (String) model.getValueAt(selecRow, 0);// 获取选中的员工编号
        ...
        back.addActionListener(new ActionListener() {// 返回按钮的事件
            public void actionPerformed(ActionEvent e) { // 返回按钮的事件
                parent.setPanel(new MainPanel(parent));
            }
        });
    }
    private class EmpTable extends JTable {//自定义员工信息表格类
